...
function groupMetrics(metricList) {
    var metricGroups = {};
    for (var grpname in metricList) {
        var newGrp = metricGroups[grpname.split('Module')[0]] = {};
        var thisGrp = metricList[grpname];
        for (var i = 0; i < thisGrp.length; i++) {
            var theMetric = thisGrp[i];
            var newName = theMetric.name.split('-').slice(-1)[0];
            delete theMetric.name;
            newGrp[newName] = theMetric;
        }
    }
    return metricGroups;
}

function loadMetrics(metricGroups, metricSelect) {
    var optgrps = '';
    for (var grpname in metricGroups) {
        var optgrp = '<optgroup label="' + grpname + '">';
        var grpsize = 0;
        var thisGrp = metricGroups[grpname];
        for (var metricName in thisGrp) {
            if (thisGrp[metricName]['enabled']) {
                optgrp += '<option value="' + 
                    metricName + '">' +
                    thisGrp[metricName]['title'] + 
                    '</option>';
                grpsize += 1;
            }
        }

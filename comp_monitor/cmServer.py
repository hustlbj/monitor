...
config_logging(config["LOG_PATH"] + os.sep + "cmServer.log")
logger = getlogger("cmServer")
...
class Server():
	...
	def __init__(self):
		self.local_ip = config['SERVER_IP'] or get_local_ip()
		self.addr = (self.local_ip, int(config['SERVER_PORT']))
		self.db = DBWrapper(config['DB_IP'],\
				 int(config['DB_PORT']), \
				 config['DB_USER'], \
				 config['DB_PWD'], \
				 config['DB_NAME'])
	...
	def start(self):
		...
		logger.info("Host: " + self.local_ip)
